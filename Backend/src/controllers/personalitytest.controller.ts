...
export const submitPersonalityTest = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (typeformResponseId) {
      try {
        verifiedResponse = await typeformService.getResponse(typeformResponseId);
        if (!verifiedResponse) {
...
export const getPersonalityTestStatus = async (req: AuthRequest, res: Response) => {
  try {
    const user = req.user;
...
export const handleTypeformWebhook = async (req: Request, res: Response) => {
  try {
    ...
    if (signature && !typeformService.verifyWebhookSignature(payload, signature)) {
    ...
    const parsedResults = typeformService.parsePersonalityTestResults(form_response);
...
export const syncTypeformResponses = async (req: Request, res: Response) => {
  try {
    const { since, until } = req.query;
    
    const responses = await typeformService.getAllResponses({
    ...
    for (const response of responses) {
      try {
        const testResults = typeformService.parsePersonalityTestResults(response);
...
export const getWebhookStatus = async (req: Request, res: Response) => {
  try {
    const webhook = await typeformService.getWebhook();
    res.json({
      success: true,
      data: webhook
    });
...
export const setupWebhook = async (req: Request, res: Response) => {
  try {
    ...
    const webhook = await typeformService.createWebhook(webhookUrl, true);
    res.json({
      success: true,
