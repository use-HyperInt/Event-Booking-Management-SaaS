...
exports.getAllUsers = exports.verifyAdmin = exports.getDashboardStats = exports.updateBookingStatus = exports.getAllBookings = exports.deleteEvent = exports.updateEvent = exports.createEvent = exports.getAdminProfile = exports.registerAdmin = void 0;
...
const registerAdmin = async (req, res) => {
    try {
        ...
        const { error } = schema_1.adminRegistrationSchema.validate(req.body);
        if (error) {
...
const createEvent = async (req, res) => {
    try {
        ...
        if (eventData.experienceTicketPrice)
            eventData.experienceTicketPrice = parseFloat(eventData.experienceTicketPrice);
        if (eventData.discountedPrice)
            eventData.discountedPrice = parseFloat(eventData.discountedPrice);
        if (eventData.feedbackEnabled)
            eventData.feedbackEnabled = eventData.feedbackEnabled === 'true';
        ...
        const { error } = schema_1.eventCreationSchema.validate(eventData);
        if (error) {
        ...
        if (req.files && Array.isArray(req.files) && req.files.length > 0) {
            try {
                const uploadPromises = req.files.map(async (file, index) => {
                    ...
                    const result = await (0, cloudinary_config_1.uploadToCloudinary)(file.buffer, 'events', fileName);
                    return result.url;
                });
        ...
        const event = new events_model_1.default(finalEventData);
        await event.save();
        res.status(201).json({
...
const updateEvent = async (req, res) => {
    try {
        ...
        const updateValidationSchema = schema_1.eventCreationSchema.fork(['title', 'description', 'understandContent', 'experienceTicketContent', 'category', 'startTime', 'endTime', 'capacity', 'experienceTicketPrice', 'price', 'eventLocation'], (schema) => schema.optional());
        ...
        const existingEvent = await events_model_1.default.findById(req.params.id);
        if (!existingEvent) {
        ...
        const event = await events_model_1.default.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
        res.json({
...
const deleteEvent = async (req, res) => {
    try {
        const event = await events_model_1.default.findByIdAndUpdate(req.params.id, { status: 'CANCELLED' }, { new: true });
        if (!event) {
            res.status(404).json({ error: 'Event not found' });
            return;
        }
...
const getAllBookings = async (req, res) => {
    try {
        ...
        const total = await booking_model_1.default.countDocuments(filter);
        res.json({
            bookings,
...
const updateBookingStatus = async (req, res) => {
    try {
        ...
        const updatedBooking = await booking_model_1.default.findByIdAndUpdate(bookingId, {
...
const getDashboardStats = async (req, res) => {
    try {
        const totalEvents = await events_model_1.default.countDocuments();
        const activeEvents = await events_model_1.default.countDocuments({ status: { $in: ['CONFIRMED', 'UPCOMING'] } });
        const totalUsers = await userprof_model_1.default.countDocuments();
        const totalBookings = await booking_model_1.default.countDocuments();
        const pendingBookings = await booking_model_1.default.countDocuments({ bookingStatus: 'pending' });
