...
exports.handleRazorpayWebhook = exports.getPaymentStatus = exports.handlePaymentFailure = exports.verifyPayment = exports.createPaymentOrder = void 0;
...
const verifyPayment = async (req, res) => {
    try {
        ...
        // Find payment record
        const payment = await payment_model_1.default.findOne({ razorpayOrderId: razorpay_order_id });
        if (!payment) {
            return res.status(404).json({ error: "Payment record not found" });
        }
        // Find booking
        const booking = await booking_model_1.default.findById(bookingId).populate("eventId");
        if (!booking) {
...
const getPaymentStatus = async (req, res) => {
    try {
        ...
        const payment = await payment_model_1.default.findOne({ bookingId });
        res.json({
            success: true,
            data: {
                booking: {
...
async function handlePaymentCaptured(paymentData) {
    try {
        const payment = await payment_model_1.default.findOne({ razorpayOrderId: paymentData.order_id });
        if (!payment) {
